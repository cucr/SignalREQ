    ...
    public class TcpClientEx
    {
        ...
        private void RecvProc()
        {
            ...
            while (m_bRunning)
            {
                //提取包长
                int dataLen = 0;
                if (CommunicationCommon.ReadAllData(m_netWorkStream, header, 5))
                {
                    dataLen = BitConverter.ToInt32(header, 1);
                    if (dataLen < 0)
                    {
                        break;//非法数据，关闭客户端
                    }
        ...
        public bool Connect(string strIp, UInt16 nPort, int timeOut)
        {
            ...
            try
            {
                CommunicationCommon.SetKeepAlive(m_tcpClient.Client, 4000, 2000);
